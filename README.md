## Quantnode Masternode Installation
...
```bash
cd ~
git clone https://github.com/quantdev1/QuantnodeMasternodeSetup.git
```
...
__publicipaddress:27001__ - this must be your masternode public IP address, which is usually the IP address of your VPS, accessible from the Internet. The new script (v1.1) will detect your IP address automatically. The __:27001__ suffix is the predefined and fixed TCP port which is being used in the Quantnode network for node-to-node and wallet-to-node communications. This port needs to be opened on your VPS server firewall so that others can talk to your masternode. The setup script takes care of it. NOTE: some VPS service providers may have additional firewall on their network which you may need to configure to open TCP port 27001. Vultr does not require this.
...
```bash
quantnode-cli masternode status

quantnode-cli mnsync status
```
...
```bash
sudo tail -f ~/.quantnode/debug.log
```

And for those who wonder what does **quantnode.conf** file looks like for a typical masternode which the setup script generates, here's an example below...
...
```bash
cat <<EOF > ~/.quantnode/quantnode.conf
rpcuser=$rpcuser
...
The script adds a cron job which starts quantnoded daemon upon reboot. Try restarting your VPS server (just type reboot in Linux console) and see if your masternode comes back online automatically in a few minutes. Log back in using PuTTY and run the following command to monitor your masternode status:

```
watch -n 10 'quantnode-cli masternode status && quantnode-cli mnsync status'
```
...
**Advanced masternode monitoring script: quantnodemon.sh**
...
```
===========================================================================
Outbound connections to other Quantnode Network nodes [quantnode datadir: /root/.quantnode]
===========================================================================
...
===========================================================================
Masternode Status:
# REEF-cli -datadir=/root/.quantnode masternode status
{
...
===========================================================================
Sync Status:
# quantnode-cli -datadir=/root/.quantnode mnsync status
{
  "AssetID": 999,
  "AssetName": "MASTERNODE_SYNC_FINISHED",
  "Attempt": 0,
...
Usage: quantnodemon.sh [refresh delay] [datadir index]
Example: quantnodemon.sh 10 22 will run every 10 seconds and query quantnoded in /root/.quantnode22
