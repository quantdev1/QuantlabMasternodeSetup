#!/bin/bash
# Quantlab Masternode Setup Script V1.4 for Ubuntu 16.04 LTS
#
...
function stop_daemon {
    if pgrep -x 'quantlabd' > /dev/null; then
        echo -e "${YELLOW}Attempting to stop Quantlabd${NC}"
        quantlab-cli stop
        sleep 30
        if pgrep -x 'quantlab' > /dev/null; then
            echo -e "${RED}Quantlabd daemon is still running!${NC} \a"
            echo -e "${RED}Attempting to kill...${NC}"
            pkill quantlabd
            sleep 30
            if pgrep -x 'quantlabd' > /dev/null; then
                echo -e "${RED}Can't stop quantlabd! Reboot and try again...${NC} \a"
                exit 2
            fi
        fi
    fi
...
echo -e "${YELLOW}Quantlab Masternode Setup Script V1.5 for Ubuntu 16.04 LTS${NC}"
...
rm -rf Ubuntu-16.04-compiled-quantlab.tar.gz
 ...
 sudo cp ~/QuantlabMasternodeSetup/quantlab/quantlab* /usr/bin/
 sudo chmod 755 -R ~/QuantlabMasternodeSetup
 sudo chmod 755 /usr/bin/quantlab*

 # Deploy masternode monitoring script
 cp ~/QuantlabMasternodeSetup/quantlabmon.sh /usr/local/bin
 sudo chmod 711 /usr/local/bin/quantlabmon.sh

 #Create datadir
 if [ ! -f ~/.quantlab/quantlab.conf ]; then
 	sudo mkdir ~/.quantlab
 fi
clear
echo -e "${YELLOW}Creating quantlab.conf...${NC}"
...
EOF
    ...
    sudo chmod 755 -R ~/.quantlab/quantlab.conf
...
echo -ne '\n'
    ...
    genkey=$(quantlab-cli masternode genkey)
    if [ -z "$genkey" ]; then
    ...
    #Stopping daemon to create quantlab.conf
    stop_daemon
    sleep .5
fi
# Create quantlab.conf
cat <<EOF > ~/.quantlab/quantlab.conf
rpcuser=$rpcuser
rpcpassword=$rpcpassword
...
#Finally, starting daemon with new quantlab.conf
quantlabd --daemon
sleep 5

#Setting auto start cron job for quantlabd
cronjob="@reboot sleep 30 && quantlabd"
crontab -l > tempcron
...
To introduce your new masternode to the Quantlab network, you need to
...
Currently your masternode is syncing with the Quantlab network...
...
========================================================================
To view masternode configuration produced by this script in quantlab.conf:
${YELLOW}cat ~/.quantlab/quantlab.conf${NC}
Here is your quantlab.conf generated by this script:
-------------------------------------------------${YELLOW}"
cat ~/.quantlab/quantlab.conf
echo -e "${NC}-------------------------------------------------
NOTE: To edit quantlab.conf, first stop the quantlabd daemon,
then edit the quantlab.conf file and save it in nano: (Ctrl-X + Y + Enter),
then start the quantlabd daemon back up:
             to stop:   ${YELLOW}quantlab-cli stop${NC}
             to edit:   ${YELLOW}nano ~/.quantlab/quantlab.conf${NC}
             to start:  ${YELLOW}quantlabd${NC}
========================================================================
To view quantlab debug log showing all MN network activity in realtime:
             ${YELLOW}tail -f ~/.quantlab/debug.log${NC}
========================================================================
...
sync status etc. in real-time, run the nodemon.sh script:
                 ${YELLOW}quantlabmon.sh${NC}
or just type 'node' and hit <TAB> to auto-complete script name.
========================================================================
Enjoy your Quantlab Masternode and thanks for using this setup script!
"
sleep 30
# Run quantlabmon.sh
quantlabmon.sh
